% Function to search through various radii for the Schwarszchild metric
format longg
G = 6.674*10^-11;
c = 2.99792*10^8;
...
for i = 1:length(sRadius)
    ...
    SM = metricGet_Schwarszchild(sRadius(i),gridSize(1));
    Z{i} = met2den(SM);
...
box on
set(gcf,'color','w');
sgtitle(['Schwarszchild Metric: R = \{' num2str(min(sRadius)) ', ' num2str(max(sRadius)) '\}, World Size = ' num2str(WorldSize) ', Res = ' num2str(spatialResolution)])
